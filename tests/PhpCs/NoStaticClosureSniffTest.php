...
final class NoStaticClosureSniffTest extends TestCase
{
    public function testAcceptsNonStaticClosuresAndNamedStaticMethods(): void
    {
        $file = $this->lint(<<<'PHP'
            <?php
            ...
            $closure = function (string $value): string {
                return $value;
            };
        ...
        self::assertSame(0, $file->getErrorCount(), json_encode($file->getErrors(), \JSON_THROW_ON_ERROR));
    }

    public function testRejectsAndFixesStaticClosures(): void
    {
        $file = $this->lint(<<<'PHP'
            <?php
            ...
            $arrow = static fn (string $value): string => $value;
            $closure = static function (string $value): string {
                return $value;
            };
            PHP);

        self::assertSame(2, $file->getErrorCount(), json_encode($file->getErrors(), \JSON_THROW_ON_ERROR));
