...
- **Datasets are seeded through the DAL under test** (deterministic ids, fixed data, idempotent upserts) and snapshotted into a cache keyed by `(DAL fingerprint × tier × engine × seeder version)`. Tiers: S / M / L. The snapshot cache is load-bearing — L-tier seeding takes hours and must happen at most once per implementation. Snapshots are archives of the server's data directory taken after a clean shutdown, so restoring one costs a copy rather than replaying every insert and rebuilding every index; a restored server starts with a cold buffer pool, which the per-cell warmup absorbs.
