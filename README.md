...
- **Datasets are seeded through the DAL under test** (deterministic ids, fixed data, idempotent upserts) and snapshotted into a cache keyed by `(DAL fingerprint × tier × engine × seeder version)`. Tiers: S / M / L. The snapshot cache is load-bearing — L-tier seeding takes hours and must happen at most once per implementation. Snapshots are archives of the server's data directory taken after a clean shutdown, so restoring one costs a copy rather than replaying every insert and rebuilding every index; a restored server starts with a cold buffer pool, which the per-cell warmup absorbs. Before any cell is measured, each implementation's probe fingerprints the dataset it will be measured on — order-independent checksums over the seeded entities, translations and association mappings, excluding timestamps and installation data — and the session refuses to compare implementations whose datasets are not logically equal, naming the entity or mapping that differs. The fingerprints are stored with the run and re-checked by `dan diff`.
...
- **Provenance is frozen too.** Every manifest records the execution environment: DAN's own source fingerprint, PHP and Composer versions, host OS/CPU and effective cgroup limits, the Docker engine (version, host resources, published-port forwarding mode) and the immutable digest of every database image. Facts that cannot be discovered are recorded as unknown, never guessed. Stored profiles recorded by different DAN revisions or different image digests are still diffed, with a warning; reports end with a reproducibility table.
