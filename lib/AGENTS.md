...
- **Only genuinely shared code belongs here.** A class qualifies when both runtime packages use it at runtime — "one package plus the other package's tests" does not count; move such a class into its real owner. Current residents: the protocol vocabulary (`Tier`, `ScenarioName`, `ScenarioResultSchemaVersion`), `Collections\{Collection,Set}`, `Time\{Duration,Timestamp}`, `Filesystem\Path`.
