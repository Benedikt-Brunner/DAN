...
- [x] **Architecture tests:** `Dan\Harness\` never depends on `Shopware\*` or `Dan\Probe\`; `Dan\Probe\` only depends on `Dan\Lib\` among DAN packages; `Dan\Lib\` depends on neither consumer and on no framework at all; only `Dan\Harness\Process` may use `Symfony\Component\Process` (the original allowed list predated the ProcessRunner refactor that confined process usage there). Pest `arch()` proves the rules the root autoloader can resolve; the cross-package directions are token scans (`DependencyDirectionTest`), because arch() silently ignores namespaces absent from the root vendor dir — a `use Shopware\...` in the harness would be invisible to it.
