...
- [x] **Update CI:** quality job on PHP 8.4 (Pest + Infection gate `min-covered-msi`, started at the measured 72% wiring baseline and ratcheted to 85 when the Phase 1 property tests landed at 90%), bundle job stays on 8.2, nightly full-`src/` mutation report + high-iteration property jobs (`nightly.yml`).
...
- [x] **Eris property-based tests** replacing the hand-picked invariant sweeps: normalizer idempotence over *generated* SQL, scheduler fairness/mirroring over random `(impls × iterations × blocks)`, merge commutativity on sample multisets, artifact round-trips through JSON transport (`Protocol`, `RunManifest`, `CellResult`, `DatabaseTarget`) plus type-corrupted-payload refusal sweeps, statistics bounds/monotonicity, and the unit-level A/A property over real run directories (identical runs diff clean; changed SQL is flagged at exactly its positions). Random seeds in CI (low iterations on PRs, 2000 nightly); a failure prints the reproducing `ERIS_SEED` command and shrinking is time-boxed to 15s.
  *Done when:* old `InvariantsTest` sweeps are replaced, properties shrink failures to minimal counterexamples. Raised the covered MSI of the deterministic core from 72% to 90%; the CI gate ratcheted to 85.
- [x] **Regex fuzz-lite:** Eris adversarial-SQL generator (huge IN-lists, pathological whitespace, paren nesting, and crucially *malformed* groups — unclosed and trailing-comma shapes, since catastrophic backtracking needs a failing match) with a wall-time budget assertion on `SqlNormalizer::normalize()` plus a losslessness assertion: on PHP's default `pcre.backtrack_limit`, a catastrophic regex manifests as `preg_replace()` returning null (cast to `''`) long before it gets slow. Coverage-guided `nikic/php-fuzzer` stays an optional nightly upgrade, not scheduled.
  *Done when:* the property exists and a deliberately catastrophic regex variant fails it (verified: a nested-quantifier variant of the named-placeholder collapse fails the property via the losslessness assertion).
- [x] **Architecture tests:** `Dan\Harness\` never depends on `Shopware\*` and, among DAN namespaces, only on `Dan\Lib\`; `Dan\Probe\` likewise only on `Dan\Lib\`; `Dan\Lib\` depends on neither consumer and on no framework at all; only `Dan\Harness\Process` may use `Symfony\Component\Process` (the original allowed list predated the ProcessRunner refactor that confined process usage there). All of it is phpat rules inside the two PHPStan gates (`tests/Arch/DependencyDirectionRules.php`, `bundle/tests/Arch/DependencyDirectionRules.php`), one gate per package because each analyzes only its own vendor dir. Pest `arch()` cannot carry these rules: it resolves target namespaces through the root Composer autoloader, so a `use Shopware\...` in the harness is silently invisible to it, as is a phpdoc-only reference (both verified by injection). phpat works on PHPStan's parsed relations instead - extends, parameter/return/property types, instantiations, static calls, catches, attributes, doc types - and an *unresolvable* reference additionally fails level max as an unknown class, whatever its spelling.
  *Done when:* rules pass, and adding a forbidden import fails the gate (verified per rule by injection: Shopware→harness, probe→harness, harness→probe namespace, Symfony→lib, Process outside its boundary).
- [x] **Renderer snapshot tests:** golden-file snapshots for `MarkdownReportRenderer` over fixture `RunComparison`s (clean A/A, SQL-change, regression-with-violations, protocol-mismatch cases) in `tests/Report/snapshots/`; the violation case renders what the real `Gate\Policy` produces. `DAN_UPDATE_SNAPSHOTS=1` regenerates.
  *Done when:* report formatting changes show up as reviewable snapshot diffs.
- [x] **Cross-package contract fixture** for the `dan:execute` JSON: one golden file (`bundle/tests/Fixtures/scenario-result.v1.json` — the probe owns its output format); the probe test asserts `ScenarioResult::toArray()` — the exact payload `ExecuteScenariosCommand` writes — matches it byte-for-byte, the harness test asserts `CellResult::fromDecodedScenarioArray` parses it losslessly.
