...
- [x] **Architecture tests:** `Dan\Harness\` never depends on `Shopware\*` and, among DAN namespaces, only on `Dan\Lib\`; `Dan\Probe\` likewise only on `Dan\Lib\`; `Dan\Lib\` depends on neither consumer and on no framework at all; only `Dan\Harness\Process` may use `Symfony\Component\Process` (the original allowed list predated the ProcessRunner refactor that confined process usage there). Pest `arch()` proves the rules the root autoloader can resolve; the cross-package directions are phpat rules inside the two PHPStan gates (`tests/Arch/DependencyDirectionRules.php`, `bundle/tests/Arch/DependencyDirectionRules.php`), because arch() silently ignores namespaces absent from the root vendor dir — a `use Shopware\...` in the harness would be invisible to it. phpat works on parsed relations, needs no autoloadability, and covers what a hand-rolled scan would have to re-implement (grouped/aliased imports, attributes, doc types); an *unresolvable* reference additionally fails PHPStan level max as an unknown class, whatever its spelling.
